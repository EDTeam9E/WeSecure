...
for cl in myList:
    if cl.endswith('.DS_Store'):
        ...
        #remove the number at beginning of each image name and store it list typeName from the list classNames
...
def markPresence(name):
    with open('log.csv','a') as f:
        ...
        # now = datetime.now()
        #write the name,type,date and time in 24 hour format in the csv file in a  row
        # f.writelines(f'{name},{dType[dNames.index(name)]},{dtString}')


def notify1(mName):
    if(ctr>0):
        msg.send(1, mName)
        imageBot.main('1159079499')

def notify2(mName, imgCap):
    if(ctr>0):
        msg.send(2, mName, imgCap)
        imageBot.main('2078595269')
        imageBot.main('1159079499')
        imageBot.main('2021811736')
...
while True:
    ...
    # cv2.imwrite('Attendance.png',img)
    # imgPass='Attendance.png'
    #img = captureScreen()
    ...
    for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
        ...
        if matches[matchIndex]:
            ...
            for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
                ...
                if(ctr>1):
                    response=recieve.getText(1159079499)
                    if(response=='Y'):
